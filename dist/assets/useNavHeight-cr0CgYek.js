import{b as s,R as I}from"./vendor-ViNJc2wV.js";import{j as i}from"./index-QnFOg_vW.js";import{S as z}from"./SocialMediaButtons-D7XkHcVn.js";const R=(l=null,S={})=>{const{throttleMs:m=100,threshold:n=50,passive:t=!0}=S,[g,v]=s.useState({scrollY:0,scrollX:0,isScrolled:!1,direction:"none",velocity:0}),a=s.useRef(0),c=s.useRef(0),o=s.useRef(0),e=s.useRef(0),r=s.useRef(null),x=s.useRef(!1),F=s.useCallback(b=>{if(x.current)return;x.current=!0,window.innerWidth<=767?setTimeout(()=>{const d=performance.now(),p=b.target===document?document.documentElement:b.target,f=p.scrollTop||window.pageYOffset||0,w=p.scrollLeft||window.pageXOffset||0,u=f-a.current,y=w-c.current,E=d-o.current;let k="none";Math.abs(u)>Math.abs(y)?k=u>0?"down":u<0?"up":"none":Math.abs(y)>0&&(k=y>0?"right":"left");const O=(E>0?Math.abs(u)/E:0)*.3+e.current*.7;v({scrollY:f,scrollX:w,isScrolled:f>n,direction:k,velocity:O}),a.current=f,c.current=w,o.current=d,e.current=O,setTimeout(()=>{x.current=!1},m)},m):(r.current&&cancelAnimationFrame(r.current),r.current=requestAnimationFrame(()=>{const d=performance.now(),p=b.target===document?document.documentElement:b.target,f=p.scrollTop||window.pageYOffset||0,w=p.scrollLeft||window.pageXOffset||0,u=f-a.current,y=w-c.current,E=d-o.current;let k="none";Math.abs(u)>Math.abs(y)?k=u>0?"down":u<0?"up":"none":Math.abs(y)>0&&(k=y>0?"right":"left");const O=(E>0?Math.abs(u)/E:0)*.3+e.current*.7;v({scrollY:f,scrollX:w,isScrolled:f>n,direction:k,velocity:O}),a.current=f,c.current=w,o.current=d,e.current=O,setTimeout(()=>{x.current=!1},m)}))},[m,n]);return s.useEffect(()=>{const b=l||window,h={passive:t};return requestAnimationFrame(()=>{const d=l?l.scrollTop:window.pageYOffset||0,p=l?l.scrollLeft:window.pageXOffset||0;v({scrollY:d,scrollX:p,isScrolled:d>n,direction:"none",velocity:0}),a.current=d,c.current=p,o.current=performance.now()}),b.addEventListener("scroll",F,h),()=>{b.removeEventListener("scroll",F),r.current&&cancelAnimationFrame(r.current)}},[l,F,n]),g},M=({currentPage:l="events",scrollY:S=0,onNavigate:m=()=>{},onMenuToggle:n=()=>{}})=>{const[t,g]=s.useState(!1),[v,a]=s.useState("100vh");s.useEffect(()=>{const e=()=>{try{const r=window.visualViewport?window.visualViewport.height:window.innerHeight;a(`${r}px`)}catch{a(`${window.innerHeight}px`)}};return e(),window.addEventListener("resize",e),window.addEventListener("orientationchange",e),window.visualViewport&&window.visualViewport.addEventListener("resize",e),()=>{window.removeEventListener("resize",e),window.removeEventListener("orientationchange",e),window.visualViewport&&window.visualViewport.removeEventListener("resize",e)}},[]);const c=()=>{const e=!t;g(e),n(e)},o=e=>{m(e),g(!1),n(!1)};return s.useEffect(()=>{const e=document.body;if(t){const r=window.scrollY;e.classList.add("drawer-scroll-lock"),e.style.top=`-${r}px`}else{e.classList.remove("drawer-scroll-lock");const r=e.style.top;e.style.top="",r&&window.scrollTo(0,parseInt(r||"0")*-1)}return()=>{e.classList.remove("drawer-scroll-lock"),e.style.top=""}},[t]),i.jsxs(i.Fragment,{children:[i.jsx("style",{children:`
