import{b as c,R as E}from"./vendor-ViNJc2wV.js";import{j as i}from"./index-B4DNj0ee.js";import{S as C}from"./SocialMediaButtons-GL4wYF9t.js";const M=(a=null,k={})=>{const{throttleMs:n=100,threshold:s=50,passive:t=!0}=k,[l,d]=c.useState({scrollY:0,scrollX:0,isScrolled:!1,direction:"none",velocity:0}),o=c.useRef(0),e=c.useRef(0),r=c.useRef(0),F=c.useRef(0),x=c.useRef(null),S=c.useRef(!1),O=c.useCallback(h=>{if(S.current)return;S.current=!0,window.innerWidth<=767?setTimeout(()=>{const p=performance.now(),f=h.target===document?document.documentElement:h.target,b=f.scrollTop||window.pageYOffset||0,u=f.scrollLeft||window.pageXOffset||0,m=b-o.current,g=u-e.current,y=p-r.current;let v="none";Math.abs(m)>Math.abs(g)?v=m>0?"down":m<0?"up":"none":Math.abs(g)>0&&(v=g>0?"right":"left");const w=(y>0?Math.abs(m)/y:0)*.3+F.current*.7;d({scrollY:b,scrollX:u,isScrolled:b>s,direction:v,velocity:w}),o.current=b,e.current=u,r.current=p,F.current=w,setTimeout(()=>{S.current=!1},n)},n):(x.current&&cancelAnimationFrame(x.current),x.current=requestAnimationFrame(()=>{const p=performance.now(),f=h.target===document?document.documentElement:h.target,b=f.scrollTop||window.pageYOffset||0,u=f.scrollLeft||window.pageXOffset||0,m=b-o.current,g=u-e.current,y=p-r.current;let v="none";Math.abs(m)>Math.abs(g)?v=m>0?"down":m<0?"up":"none":Math.abs(g)>0&&(v=g>0?"right":"left");const w=(y>0?Math.abs(m)/y:0)*.3+F.current*.7;d({scrollY:b,scrollX:u,isScrolled:b>s,direction:v,velocity:w}),o.current=b,e.current=u,r.current=p,F.current=w,setTimeout(()=>{S.current=!1},n)}))},[n,s]);return c.useEffect(()=>{const h=a||window,I={passive:t};return requestAnimationFrame(()=>{const p=a?a.scrollTop:window.pageYOffset||0,f=a?a.scrollLeft:window.pageXOffset||0;d({scrollY:p,scrollX:f,isScrolled:p>s,direction:"none",velocity:0}),o.current=p,e.current=f,r.current=performance.now()}),h.addEventListener("scroll",O,I),()=>{h.removeEventListener("scroll",O),x.current&&cancelAnimationFrame(x.current)}},[a,O,s]),l},N=({currentPage:a="events",scrollY:k=0,onNavigate:n=()=>{},onMenuToggle:s=()=>{}})=>{const[t,l]=c.useState(!1),d=()=>{const e=!t;l(e),s(e)},o=e=>{n(e),l(!1),s(!1)};return c.useEffect(()=>{const e=document.body;if(t){const r=window.scrollY;e.classList.add("drawer-scroll-lock"),e.style.top=`-${r}px`}else{e.classList.remove("drawer-scroll-lock");const r=e.style.top;e.style.top="",r&&window.scrollTo(0,parseInt(r||"0")*-1)}return()=>{e.classList.remove("drawer-scroll-lock"),e.style.top=""}},[t]),i.jsxs(i.Fragment,{children:[i.jsx("style",{children:`
