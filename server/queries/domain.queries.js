...
async function find(match) {
    if (match.address && env.REDIS_ENABLED && redis.isRedisReady()) {
        try {
            const cachedDomain = await redis.safeRedisCommand('get', redis.key.domain(match.address));
            if (cachedDomain) return JSON.parse(cachedDomain);
        } catch (error) {
            console.warn('⚠️ Redis cache read failed for domain, continuing with database query:', error.message);
        }
    }

    const domain = await knex("domains").where(match).first();

    if (domain && env.REDIS_ENABLED && redis.isRedisReady()) {
        try {
            const key = redis.key.domain(domain.address);
            await redis.safeRedisCommand('setex', key, 60 * 15, JSON.stringify(domain));
        } catch (error) {
            console.warn('⚠️ Redis cache write failed for domain:', error.message);
        }
    }
