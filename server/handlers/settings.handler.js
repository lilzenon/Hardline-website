...
async function updateSettings(req, res) {
    try {
        ...
        if (default_title || default_description) {
            ...
            updatedSettings.seo = await query.seoSettings.updateSEOSettings(seoSettingsData, req.user && req.user.id);
...
async function resetSettings(req, res) {
    try {
        ...
        if (section === 'seo') {
            ...
            const resetSettings = await query.seoSettings.updateSEOSettings(defaultSeoSettings, req.user && req.user.id);
