...
router.get('/optimized/:imageName', async (req, res) => {
    try {
        ...
        if (width && bestFormat !== 'original') {
            ...
            possiblePaths.push(
                path.join(__dirname, '../../static/images/optimized', `${imageName.replace(/\.[^/.]+$/, '')}-${width}w.${bestFormat}`)
            );
        }
        ...
        if (bestFormat !== 'original') {
            ...
            possiblePaths.push(
                path.join(__dirname, '../../static/images/optimized', `${imageName.replace(/\.[^/.]+$/, '')}.${bestFormat}`)
            );
        }
        
        // Fallback to original
        possiblePaths.push(
            path.join(__dirname, '../../static/images/figma-exact', imageName)
        );
