...
class TOTPService {
  ...
  static verifyToken(token, secret, window = 2) {
    try {
      return speakeasy.totp.verify({
        ...
        encoding: 'base32',
        token: token,
        window: window, // Allow 2 time steps before/after current time
   ...
   * @param {number} windowSeconds - Time window in seconds (default: 30)
   * @returns {boolean} - True if token was recently used
   */
  static isTokenRecentlyUsed(lastUsed, windowSeconds = 30) {
